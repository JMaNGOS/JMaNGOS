...
public abstract class FieldsObject implements Streamable {
    ...
    @Id
    @GenericGenerator(name = "seq_id", strategy = "org.jmangos.realm.model.WowExtendIdGenerator", parameters = { @Parameter(name = "table_name", value = "wow_sequences") })
    ...
    @Column(name = "entry", nullable = false, insertable = true, updatable = true, length = 10, precision = 0)
    private int               entry;
    
    @Column(name = "scale", nullable = false, insertable = true, updatable = true, length = 10, precision = 0)
     ...
     * 
     * @see
     * org.jmangos.test.id.Streamable#BuildCreateUpdateBlockForPlayer(org.jmangos.test.id.Player)
     */
    @Override
    ...
    public ChannelBuffer writeValuesUpdate() {
        ...
        final ChannelBuffer ocBuffer = ChannelBuffers.dynamicBuffer(ByteOrder.LITTLE_ENDIAN, this.bitSet.size() * 32);
        final int arraySize = ((bytesc.length % 4) > 0) ? ((bytesc.length / 4) + 1) * 4 : bytesc.length;
