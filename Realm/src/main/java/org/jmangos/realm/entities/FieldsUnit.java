...
import org.jmangos.realm.model.UnitRegenModifier;
...
public class FieldsUnit extends FieldsObject {
    ...
    @OneToOne(targetEntity = CharacterPowers.class, fetch = FetchType.EAGER, cascade = CascadeType.ALL)
    CharacterPowers                      powers;
    
    @Transient
    private Map<Byte, UnitRegenModifier> powerRegenModifier  = new FastMap<Byte, UnitRegenModifier>();
    {
        // FIX for test
    ...
    @Transient
    private final float[][]              powerModifiersGroup = new float[Powers.values().length][ModType.values().length];
    ...
    public ChannelBuffer writeValuesUpdate() {
        ...
        for (final Entry<Byte, UnitRegenModifier> mod : this.powerRegenModifier.entrySet()) {
            if (this.bitSet.get(UnitField.UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER.getValue() + mod.getKey())) {
                ocBuffer.writeFloat(mod.getValue().getFlat());
            }
        }
        for (final Entry<Byte, UnitRegenModifier> mod : this.powerRegenModifier.entrySet()) {
            if (this.bitSet.get(UnitField.UNIT_FIELD_POWER_REGEN_INTERRUPTED_FLAT_MODIFIER.getValue() + mod.getKey())) {
