...
public enum UnitFlags {
  ...
  UNIT_FLAG_DISABLE_MOVE          ( 1 << 2  ),
	...
	UNIT_FLAG_PVP_ATTACKABLE        ( 1 << 3  ),  // allow apply pvp rules to attackable state in addition to faction dependent state
  ...
  UNIT_FLAG_OOC_NOT_ATTACKABLE    ( 1 << 8  ),  // 2.0.8 - (OOC Out Of Combat) Can not be attacked when not in combat. Removed if unit for some reason enter combat (flag probably removed for the attacked and it's party/group only)
	/** The UNI t_ fla g_ passive. */
  UNIT_FLAG_PASSIVE               ( 1 << 9  ),  // makes you unable to attack everything. Almost identical to our "civilian"-term. Will ignore it's surroundings and not engage in combat unless "called upon" or engaged by another unit.
  ...
  UNIT_FLAG_IN_COMBAT             ( 1 << 19 ),
	...
	UNIT_FLAG_TAXI_FLIGHT           ( 1 << 20 ),  // disable casting at client side spell not allowed by taxi flight (mounted?), probably used with 0x4 flag
	/** The UNI t_ fla g_ disarmed. */
  UNIT_FLAG_DISARMED              ( 1 << 21 ),  // 3.0.3, disable melee spells casting..., "Required melee weapon" added to melee spells tooltip.
  ...
  UNIT_FLAG_CONFUSED              ( 1 << 22 ),
	...
	UNIT_FLAG_PLAYER_CONTROLLED     ( 1 << 24 ),  // used in spell Eyes of the Beast for pet... let attack by controlled creature
