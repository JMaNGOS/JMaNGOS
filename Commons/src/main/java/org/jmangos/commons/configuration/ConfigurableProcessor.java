...
public class ConfigurableProcessor {
	...
	private static void process(Class<? extends Object> clazz, Object obj,
		...
		if (obj == null) {
			for (Class<? extends Object> itf : clazz.getInterfaces()) {
				process(itf, null, props);
			}
		}
			...
			if (f.isAnnotationPresent(Property.class)) {
				...
				if (Modifier.isFinal(f.getModifiers())) {
					RuntimeException re = new RuntimeException(
							"Attempt to proceed final field " + f.getName()
	...
	private static void processField(Field f, Object obj, Properties[] props) {
		...
		try {
			...
			if (!Property.DEFAULT_VALUE.equals(property.defaultValue())
					|| isKeyPresent(property.key(), props)) {
				f.set(obj, getFieldValue(f, props));
			} else if (log.isDebugEnabled()) {
				log.debug("Field " + f.getName() + " of class "
						+ f.getDeclaringClass().getName() + " wasn't modified");
			}
		} catch (Exception e) {
			RuntimeException re = new RuntimeException("Can't transform field "
					+ f.getName() + " of " + f.getDeclaringClass(), e);
			log.error(re);
			throw re;
		}
	 ...
	 */
	private static Object getFieldValue(Field field, Properties[] props)
			throws TransformationException {
		...
		if (key.isEmpty()) {
			log.warn("Property " + field.getName() + " of class "
					+ field.getDeclaringClass().getName() + " has empty key");
		} else {
		...
		if (value == null) {
			value = defaultValue;
			log.warn("Using default value for field " + field.getName()
