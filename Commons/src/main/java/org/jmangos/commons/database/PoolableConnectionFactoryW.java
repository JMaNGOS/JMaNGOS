...
public class PoolableConnectionFactoryW extends PoolableConnectionFactory {
    ...
    /**
     * Validation time out - how much seconds driver should wait for response from server. 0 -
     * indicates there are no timeout -1 - indicates that validation should not be used.
     * 
     */
     ...
     *            the {@link KeyedObjectPoolFactory} to use to create {@link KeyedObjectPool}s for
     *            pooling
     * @param validationTimeout
     *            a timeout value in seconds used to {@link #validateObject validate}
     *            {@link Connection}s. Value of <tt>0</tt> means no timeout. Using <tt>-1</tt> turns
     *            off validation.
     ...
     */
    public PoolableConnectionFactoryW(final ConnectionFactory connFactory, final ObjectPool pool, final KeyedObjectPoolFactory stmtPoolFactory,
            final int validationTimeout, final boolean defaultReadOnly, final boolean defaultAutoCommit) {
    ...
    /**
     * Validate connection by checking if connection is not closed and is still valid. throws
     * SQLException if connection is invalid.
     * 
     * @param conn
     *            the conn
     * @throws SQLException
