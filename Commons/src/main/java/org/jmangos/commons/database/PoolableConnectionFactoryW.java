...
public class PoolableConnectionFactoryW extends PoolableConnectionFactory {
	 ...
	 *            the {@link KeyedObjectPoolFactory} to use to create
	 *            {@link KeyedObjectPool}s for pooling
	 * @param validationTimeout
	 *            a timeout value in seconds used to {@link #validateObject
	 *            validate} {@link Connection}s. Value of <tt>0</tt> means no
	 *            timeout. Using <tt>-1</tt> turns off validation.
	 * @param defaultReadOnly
	 *            the default "read only" setting for borrowed
	 *            {@link Connection}s
	 * @param defaultAutoCommit
	 *            the default "auto commit" setting for returned
	 *            {@link Connection}s {@link java.sql.PreparedStatement}s, or
	 *            <tt>null</tt> to disable {@link java.sql.PreparedStatement}
	 *            pooling
	 */
	public PoolableConnectionFactoryW(ConnectionFactory connFactory,
			ObjectPool pool, KeyedObjectPoolFactory stmtPoolFactory,
			int validationTimeout, boolean defaultReadOnly,
			boolean defaultAutoCommit) {
		super(connFactory, pool, stmtPoolFactory, null, defaultReadOnly,
				defaultAutoCommit);
