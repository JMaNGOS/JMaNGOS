...
public class MTRandom extends Random {
     ...
     */
    public MTRandom(final boolean compatible) {
        super(0L);
     ...
     */
    public MTRandom(final long seed) {
        super(seed);
    }
     ...
     */
    public MTRandom(final byte[] buf) {
        super(0L);
        setSeed(buf);
    }
    ...
    private void setSeed(final int seed) {
        ...
        for (this.mti = 1; this.mti < N; this.mti++) {
            this.mt[this.mti] =
                    ((MAGIC_FACTOR1 * (this.mt[this.mti - 1] ^ (this.mt[this.mti - 1] >>> 30))) + this.mti);
        }
     ...
     * first and then use that to re-seed the instance. The behaviour of the
     * class
     ...
     *         if the buffer has zero length.
     */
    public final void setSeed(final byte[] buf) {
        setSeed(pack(buf));
    }
    ...
    public final synchronized void setSeed(final int[] buf) {
        ...
        for (; k > 0; k--) {
            this.mt[i] =
                    (this.mt[i] ^ ((this.mt[i - 1] ^ (this.mt[i - 1] >>> 30)) * MAGIC_FACTOR2)) +
                        buf[j] +
                        j;
            i++;
            j++;
        ...
        for (k = N - 1; k > 0; k--) {
            this.mt[i] =
                    (this.mt[i] ^ ((this.mt[i - 1] ^ (this.mt[i - 1] >>> 30)) * MAGIC_FACTOR3)) - i;
            i++;
            if (i >= N) {
                this.mt[0] = this.mt[N - 1];
                i = 1;
        ...
        this.mt[0] = UPPER_MASK; // MSB is 1; assuring non-zero initial array
        // ---- End Mersenne Twister Algorithm ----
    }
     ...
     * sequence
     * from this class. If given a value of 32, this method behaves identically
     * to
    ...
    protected final synchronized int next(final int bits) {
        ...
        if (this.mti >= N) { // generate N words at one time
            ...
            for (kk = 0; kk < (N - M); kk++) {
                ...
                this.mt[kk] = this.mt[kk + M] ^ (y >>> 1) ^ MAGIC[y & 0x1];
            }
            for (; kk < (N - 1); kk++) {
                y = (this.mt[kk] & UPPER_MASK) | (this.mt[kk + 1] & LOWER_MASK);
                this.mt[kk] = this.mt[kk + (M - N)] ^ (y >>> 1) ^ MAGIC[y & 0x1];
            }
            y = (this.mt[N - 1] & UPPER_MASK) | (this.mt[0] & LOWER_MASK);
            this.mt[N - 1] = this.mt[M - 1] ^ (y >>> 1) ^ MAGIC[y & 0x1];
     ...
     * packing the byte array into an integer array first, using this method,
     * and
     * then invoking setSeed() with that; it removes the need to re-pack the
     * byte
     ...
     */
    public static int[] pack(final byte[] buf) {
        int k;
