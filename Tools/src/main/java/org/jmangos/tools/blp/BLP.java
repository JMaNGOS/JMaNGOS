...
public class BLP {
	...
	public int type; // 0 = JPEG compression; 1 = Uncompressed or DirectX
						// compression
	public byte encoding; // 1 for uncompressed, 2 for DirectX compression
	public byte alphaBitDepth; // 0, 1 or 8
	public byte alphaEncoding; // 0 = DirectX 1 alpha (0/1 bit); 1 = DirectX 2/3
								// alpha (4-bit alpha); 2 = DirectX 4/5 alpha
								// (interpolated)
	public byte hasMips; // 0 = no mip maps; 1 = has mips (# determined by
							// dimensions)
	public int width;
	public int height;
	...
	public String toString() {
		return "(" + width + "x" + height + ") " + type + "/" + encoding + "/"
				+ alphaBitDepth + "/" + alphaEncoding;
	}

	private static final void readMipMaps(BLP result, final ByteBuffer bb) {
		for (int index = 0; index < result.mipmapOffsets.length; index++) {
			if ((result.mipmapOffsets[index] > 0)
					&& (result.mipmapOffsets[index] < bb.capacity())) {
	...
	protected BufferedImage getBufferedImageUncompressed(ByteBuffer bb,
			int alphaBitDepth) {
		...
		for (int i = 0; i < numTilesHigh; i++) {
			for (int j = 0; j < numTilesWide; j++) {
				...
				for (int k = pixels.length - 1; k >= 0; k--) {
					...
					pixels[k] = (alphaValue << 24)
							| getPixel888(lookupTable[colorCode]);
				}
	...
	private static Color[] expandLookupTableDXT1(short c0, short c1,
			boolean alpha) {
		int uC0 = c0;
		int uC1 = c1;
