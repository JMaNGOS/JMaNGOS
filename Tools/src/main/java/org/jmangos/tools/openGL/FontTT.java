 ...
 * 
 *         This module utilizes the modules Texture and TextureLoader in order to load and store
 *         texture information. The most complicated thing to know about these classes is that
 *         TextureLoader takes the BufferedImage and converts it into a Texture. If an image is not
 *         "power of 2" Textureloader makes it a power of 2 and sets the texture coordinates
 *         appropriately.
 * 
 */
public class FontTT {
    ...
    /*
     * Pass in the preloaded truetype font, the resolution at which you wish the initial texture to
    ...
    /*
     * Create a standard Java2D bufferedimage for the font outline to later be converted into a
     * texture
     */
    ...
    /*
     * Draws the given characters to the screen size = size of the font (does not change resolution)
     * x,y,z = position to draw at color = color of font to draw rotx, roty, rotz = how much to
     * rotate the font on each axis centered = center the font at the given location, or left
     * justify
     */
    public void drawText(final String whatchars, final float size, final float x, final float y, final float z, final Color color, final float rotxpass,
        ...
        for (int i = 0; i < whatchars.length(); i++) {
            ...
            drawtexture(this.charactersp[k], fontsizeratio, totalwidth, 0, color, rotxpass, rotypass, rotzpass);
    ...
    /*
     * Draws the given characters to the screen with a drop shadow size = size of the font (does not
     * change resolution) x,y,z = position to draw at color = color of font to draw shadowcolor =
     * color of the drop shadow rotx, roty, rotz = how much to rotate the font on each axis centered
     * = center the font at the given location, or left justify
     */
    public void drawText(final String whatchars, final float size, final float x, final float y, final float z, final Color color, final Color shadowcolor,
    ...
    /*
     * Draws the given characters to the screen size = size of the font (does not change resolution)
     * x,y,z = position to draw at color = color of font to draw outlinecolor = color of the font's
     * outline rotx, roty, rotz = how much to rotate the font on each axis centered = center the
     * font at the given location, or left justify
     */
    public void drawOutlinedText(final String whatchars, final float size, final float x, final float y, final float z, final Color color,
            final Color outlinecolor, final float rotxpass, final float rotypass, final float rotzpass, final boolean centered) {
        ...
        for (int i = 0; i < whatchars.length(); i++) {
            ...
            drawtexture(this.characterso[ko], fontsizeratio, totalwidth, 0, outlinecolor, rotxpass, rotypass, rotzpass);
            
            k = ((this.charlistp.get(tempstr))).charnum;
            xoffset = ((this.characterso[k].getImageWidth() - this.charactersp[k].getImageWidth()) * fontsizeratio) / 2f;
            yoffset = ((this.characterso[k].getImageHeight() - this.charactersp[k].getImageHeight()) * fontsizeratio) / 2f;
            drawtexture(this.charactersp[k], fontsizeratio, totalwidth + xoffset, yoffset, color, rotxpass, rotypass, rotzpass);
     ...
     */
    private void drawtexture(final Texture texture, final float ratio, final float x, final float y, final Color color, final float rotx, final float roty,
            final float rotz) {
    ...
    /*
     * Returns the width in pixels of the given string, size, outlined or not used for determining
    ...
    /*
     * For convenience of checking user input keys Can be taken out if you're not going to use it
     */
    public boolean keyrangevalid(final int currentKey) {
    
        boolean retvalue = false;
        if ((currentKey == Keyboard.KEY_A) || (currentKey == Keyboard.KEY_B) || (currentKey == Keyboard.KEY_C) || (currentKey == Keyboard.KEY_D)
                || (currentKey == Keyboard.KEY_E) || (currentKey == Keyboard.KEY_F) || (currentKey == Keyboard.KEY_G) || (currentKey == Keyboard.KEY_H)
                || (currentKey == Keyboard.KEY_I) || (currentKey == Keyboard.KEY_J) || (currentKey == Keyboard.KEY_K) || (currentKey == Keyboard.KEY_L)
                || (currentKey == Keyboard.KEY_M) || (currentKey == Keyboard.KEY_N) || (currentKey == Keyboard.KEY_O) || (currentKey == Keyboard.KEY_P)
                || (currentKey == Keyboard.KEY_Q) || (currentKey == Keyboard.KEY_R) || (currentKey == Keyboard.KEY_S) || (currentKey == Keyboard.KEY_T)
                || (currentKey == Keyboard.KEY_U) || (currentKey == Keyboard.KEY_V) || (currentKey == Keyboard.KEY_W) || (currentKey == Keyboard.KEY_X)
                || (currentKey == Keyboard.KEY_Y) || (currentKey == Keyboard.KEY_Z) || (currentKey == Keyboard.KEY_0) || (currentKey == Keyboard.KEY_1)
                || (currentKey == Keyboard.KEY_2) || (currentKey == Keyboard.KEY_3) || (currentKey == Keyboard.KEY_4) || (currentKey == Keyboard.KEY_5)
                || (currentKey == Keyboard.KEY_6) || (currentKey == Keyboard.KEY_7) || (currentKey == Keyboard.KEY_8) || (currentKey == Keyboard.KEY_9)
                || (currentKey == Keyboard.KEY_PERIOD) || (currentKey == Keyboard.KEY_SPACE) || (currentKey == Keyboard.KEY_RETURN)
                || (currentKey == Keyboard.KEY_COMMA) || (currentKey == Keyboard.KEY_SLASH) || (currentKey == Keyboard.KEY_SEMICOLON)
                || (currentKey == Keyboard.KEY_LBRACKET) || (currentKey == Keyboard.KEY_RBRACKET) || (currentKey == Keyboard.KEY_EQUALS)
                || (currentKey == Keyboard.KEY_MINUS) || (currentKey == Keyboard.KEY_APOSTROPHE) || (currentKey == Keyboard.KEY_BACK)) {
            retvalue = true;
        }
        return retvalue;
    }
    ...
    public boolean keyrangevalidnumbers(final int currentKey) {
        ...
        boolean retvalue = false;
        if ((currentKey == Keyboard.KEY_0) || (currentKey == Keyboard.KEY_1) || (currentKey == Keyboard.KEY_2) || (currentKey == Keyboard.KEY_3)
                || (currentKey == Keyboard.KEY_4) || (currentKey == Keyboard.KEY_5) || (currentKey == Keyboard.KEY_6) || (currentKey == Keyboard.KEY_7)
                || (currentKey == Keyboard.KEY_8) || (currentKey == Keyboard.KEY_9) || (currentKey == Keyboard.KEY_PERIOD) || (currentKey == Keyboard.KEY_BACK)) {
            retvalue = true;
        }
        return retvalue;
    }
