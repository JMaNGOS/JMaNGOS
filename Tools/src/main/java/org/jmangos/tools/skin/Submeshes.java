...
public class Submeshes {
    ...
    /*
     * 0x00 uint32 ID Mesh part ID, see below. 0x04 uint16 StartVertex Starting vertex number. 0x06
     * uint16 nVertices Number of vertices. 0x08 uint16 StartTriangle Starting triangle index
     * (that's 3* the number of triangles drawn so far). 0x0A uint16 nTriangles Number of triangle
     * indices. 0x0C uint16 nBones Number of elements in the bone lookup table. 0x0E uint16
     * StartBones Starting index in the bone lookup table. 0x10 uint16 Unknown 0x12 uint16 RootBone
     * Not sure. 0x14 Vec3F CenterMass Average position of all the vertices in the submesh. 0x20
     * Vec3F CenterBoundingBox The center of the box when an axis aligned box is built around the
    ...
    public String toString() {
        ...
        return "[Submeshes]\n\tID = " + this.ID + "\n\tnStartVertex: " + this.StartVertex + "\n\tnVertices: " + this.nVertices + "\n\tStartTriangle: "
                + this.StartTriangle + "\n\tnTriangles: " + this.nTriangles + "\n\tnBones: " + this.nBones + "\n\tUnknown: " + this.Unknown + "\n\tRadius: "
                + this.Radius;
    }
}
